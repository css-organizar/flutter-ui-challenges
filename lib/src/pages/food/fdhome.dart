...
class FoodDeliveryHomePage extends StatelessWidget {
  ...
  final List<Map> restaurants = [
    {"image": burger, "name": "Burger King", "specials": "Vegetarian, Continental"},
      ...
      child: Stack(
        children: <Widget>[
          ...
          Positioned(
            bottom: 20,
            left: 20,
            child: Text("Heavy discount on meals today only.", style: TextStyle(color: Colors.white)),
          )
        ],
      ),
    );
      ...
      delegate: SliverChildBuilderDelegate((BuildContext context, int index) {
        return Container(
          ...
          child: Column(
            ...
            children: <Widget>[
              Container(
                  height: 150.0, width: double.infinity, child: PNetworkImage(sliderItems[index], fit: BoxFit.cover)),
              SizedBox(
                height: 10.0,
              ),
              Row(
